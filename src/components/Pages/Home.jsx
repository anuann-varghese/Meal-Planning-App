...
function Home({recipes,setRecipes,recipeId,setRecipeId,mealPlan}) {
  ...
  <label htmlFor="my-drawer-2" className="btn btn-secondary drawer-button lg:hidden">View Your Meal Plan</label>
    {/* Page content here */}
   {recipes&&recipes.map((item,k)=><div  key={k}  className=' ' >
  ...
  <div className="card-body">
    ...
    <div className="card-actions justify-end">
      <button onClick={()=>getRecipeId(item.idMeal)} className="btn btn-primary">View Detail</button>
    </div>
  </div>
</div>
   </div>)}
  ...
  <div className="drawer-side ">
    ...
    <ul className="menu p-4 w-80 min-h-full flex gap-8 bg-base-200 text-base-content">
      ...
      <div key={j} className='' >
        <li><img width={50} src={i.strMealThumb} alt="" /> <p>{i.strMeal}</p></li>
      </div>)}
